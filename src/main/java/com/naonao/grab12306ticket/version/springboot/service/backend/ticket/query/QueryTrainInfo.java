package com.naonao.grab12306ticket.version.springboot.service.backend.ticket.query;

import com.alibaba.fastjson.JSONObject;
import com.naonao.grab12306ticket.version.database.backend.constants.ConvertMap;
...
import com.naonao.grab12306ticket.version.database.backend.resultclass.ticket.query.TrainInfo;
import com.naonao.grab12306ticket.version.database.backend.scheduler.arguments.QueryTrainInfoArguments;
import com.naonao.grab12306ticket.version.database.backend.ticket.query.common.AbstractQuery;
...
public class QueryTrainInfo extends AbstractQuery {
    ...
    /**
     * use to test, make a QueryTrainInfoReturnResult object
     * @return QueryTrainInfoReturnResult
     */
    private QueryTrainInfoReturnResult makeTestQueryTrainInfoReturnResult(){
        QueryTrainInfoReturnResult queryTrainInfoReturnResult = new QueryTrainInfoReturnResult();
        queryTrainInfoReturnResult.setStatus(true);
        queryTrainInfoReturnResult.setSession(session);
        queryTrainInfoReturnResult.setMessage(FIND_ELIGIBLE_TRAIN);
        queryTrainInfoReturnResult.setTrainInfoList(new ArrayList<>());
        queryTrainInfoReturnResult.setUserInformationTable(userInformationTable);
        queryTrainInfoReturnResult.setGrabTicketInformationTable(grabTicketInformationTable);
        queryTrainInfoReturnResult.setNotifyInformationTable(notifyInformationTable);
        queryTrainInfoReturnResult.setStatusInformationTable(statusInformationTable);
